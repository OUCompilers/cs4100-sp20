...
0. Accept the assignment at [https://classroom.github.com/g/m8aojKaz](https://classroom.github.com/g/m8aojKaz).
...
3. You must use one of the following languages to implement `vm`: Rust, C, C++, Python, OCaml. In order for us to support submissions in multiple languages, we ask that *all* submissions contain at least the following three files:
   ...
   src/main.<rs|c|cc|py|ml> <-- Your project's root file
   ```   
If you use a language other than Rust, it's your responsibility to update the targets in `Makefile` accordingly so that CI passes.
